...
if __name__ == "__main__":
    ...
    retry_failed_images("failed_images.csv", "test_images")
...
print("Loading SFR-Embedding-Mistral (7B)...")
...
print("SFR-Embedding-Mistral loaded successfully.")
...
def precompute_text_embeddings(df, batch_size=2):
    ...
    with tqdm(total=total_batches, desc="Text Embeddings", leave=True) as pbar:
        ...
        for i in range(0, len(df), batch_size):
            ...
            texts = [row["clean_text"] for _, row in batch.iterrows()]
...
}, "train_text_embeddings_SFR-Embedding-Mistral(7B).pt")

torch.save({
    "embeddings": val_text_embeddings,
    "price": torch.tensor(val_df["price"].values),
    "index": val_df.index.values
}, "val_text_embeddings_SFR-Embedding-Mistral(7B).pt")
...
print("Loading EVA-CLIP-02-E-14-plus...")
...
print("EVA-CLIP-02-Giant loaded successfully.")
...
def precompute_image_embeddings(df, image_folder, batch_size=4):
    ...
    with tqdm(total=total_batches,
        ...
        for i in range(0, len(df), batch_size):
            ...
            for img_name in batch["image_name"].values:
...
torch.save({
    ...
    "price": torch.tensor(train_df["price"].values),
    "index": train_df.index.values
}, "train_image_embeddings_EVA02-E-14-plus.pt")

torch.save({
    "embeddings": val_image_embeddings,
    "price": torch.tensor(val_df["price"].values),
    "index": val_df.index.values
}, "val_image_embeddings_EVA02-E-14-plus.pt")

print("Image embeddings saved successfully.")

import torch
train_text_data = torch.load(
    "/content/train_text_embeddings_SFR-Embedding-Mistral(7B).pt",
    weights_only=False
)
val_text_data = torch.load(
    "/content/val_text_embeddings_SFR-Embedding-Mistral(7B).pt",
    weights_only=False
)
train_image_data = torch.load(
    "/content/train_image_embeddings_EVA02-E-14-plus.pt",
    weights_only=False
)
val_image_data = torch.load(
    "/content/val_image_embeddings_EVA02-E-14-plus.pt",
    weights_only=False
)
train_text_embeddings = train_text_data["embeddings"]
val_text_embeddings   = val_text_data["embeddings"]
train_image_embeddings = train_image_data["embeddings"]
val_image_embeddings   = val_image_data["embeddings"]
...
class EmbeddingDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.price = torch.log(
            torch.tensor(df["price"].values, dtype=torch.float32) + 1
        )

        assert len(self.text) == len(self.image) == len(self.manual) == len(self.price)
